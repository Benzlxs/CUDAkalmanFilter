...
{
int success;
volatile clock_t gputime0, cputime0, temp_t;
...
temp_t = clock()- temp_t;fprintf(stderr, "allocate gpu memory time: %7.1f ms ",temp_t/1.e3f);
printf("\n ... \n");
...
temp_t = clock()- temp_t;fprintf(stderr, "move memory from CPU to GPU time: %7.1f ms ",temp_t/1.e3f);
printf("\n ... \n");
// stop timer
...
printf( "\n ... \n");
cputime0 = clock()- cputime0;fprintf(stderr, "CPU based Kalman: %7.1f ms ",cputime0/1.e3f);
...
int threadsPerBlock = 512; // 512; //256;
int Nos = no*ns;
int Ns =  ns;
int No = no;
int Ns2 = ns*ns;
...
printf( "\n ... \n");
gputime0 = clock()- gputime0;fprintf(stderr, "GPU based Kalman: %7.1f ms ",gputime0/1.e3f);
// stop GPU timer
...
cublasShutdown();
//printf( "\n ... \n");
//temp_t = clock()- temp_t;fprintf(stderr, "copy and release time: %7.1f ms ",temp_t/1.e3f);
...
//P = [I - KH]P
	...
	{
	t1 = clock() - t1;
	fprintf(stderr, "CPU inversion time:  %7.1f ms ",t1/1.e3f);
